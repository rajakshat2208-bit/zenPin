...
const _curatedCache = {};   // { category: [url, url, ...] }
let   _curatedLoaded = false;
let   _curatedLoadPromise = null;

// Folder name → category key mapping (mirrors generate-manifest.js)
const FOLDER_TO_CATKEY = {
  "cars":          "cars",
  "bikes":         "bikes",
  "anime":         "anime",
  "gaming":        "gaming",
...
async function loadCuratedManifest() {
  if (_curatedLoaded) return _curatedCache;
  if (_curatedLoadPromise) return _curatedLoadPromise;

  _curatedLoadPromise = fetch("images.json")
    .then(r => r.ok ? r.json() : null)
    .then(data => {
      if (!data) return _curatedCache;
      for (const [folder, urls] of Object.entries(data)) {
        if (folder.startsWith("_") || !Array.isArray(urls) || !urls.length) continue;
        const catKey = FOLDER_TO_CATKEY[folder] || folder.toLowerCase();
        if (!_curatedCache[catKey]) _curatedCache[catKey] = [];
        _curatedCache[catKey].push(...urls);
      }
      _curatedLoaded = true;
      const total = Object.values(_curatedCache).reduce((s, a) => s + a.length, 0);
      if (total > 0) console.log(`📸 Loaded ${total} curated images from manifest`);
      return _curatedCache;
    })
    .catch(() => _curatedCache);   // silently fail if no manifest

  return _curatedLoadPromise;
}

// Convert curated URLs into idea-card objects for renderGrid
function curatedUrlsToIdeas(urls, category, startId = 0) {
  ...
  return urls.map((url, i) => ({
    id:          -(startId + i + 1),   // negative IDs = curated, never clash with DB
...
// Get curated images for a category, shuffled for variety
function getCuratedForCategory(category, limit = 12, page = 1) {
  const catKey = category.toLowerCase();
  const urls   = _curatedCache[catKey] || [];
  if (!urls.length) return [];
  // Deterministic shuffle per page using seed
  const seed    = page * 17 + catKey.charCodeAt(0);
  const shuffled = [...urls].sort((a, b) => {
    const ha = (a.charCodeAt(0) * seed) % 97;
    const hb = (b.charCodeAt(0) * seed) % 97;
    return ha - hb;
  });
  const start = ((page - 1) * limit) % shuffled.length;
  const slice = [];
  for (let i = 0; i < limit; i++) {
    slice.push(shuffled[(start + i) % shuffled.length]);
  }
  return curatedUrlsToIdeas(slice, category, (page - 1) * limit);
}

// Load manifest at startup (non-blocking)
loadCuratedManifest();
